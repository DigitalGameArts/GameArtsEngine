...
vector<int> Light::avaiableLights;
...
void Light::Initialize(void){
	...
	for (int i = 0; i < numLights; i++){
		avaiableLights.push_back(GL_LIGHT0 + i);
	}
...
Light::Light(LIGHT_TYPE type){
	...
	if (avaiableLights.size() > 0)
	{
		lightNum = avaiableLights[0];

		avaiableLights.erase(avaiableLights.begin());
...
Light::~Light(){
	if (lightNum != 0)
	{
		avaiableLights.push_back(lightNum);
	}
